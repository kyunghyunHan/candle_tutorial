...
pub fn main() -> candle_core::Result<()> {
    ...
    let w2 = Tensor::zeros(1, candle_core::DType::F32, &device)?;
    let w3 = Tensor::zeros(1, candle_core::DType::F32, &device)?;
    let wb = Tensor::zeros(1, candle_core::DType::F32, &device)?;
    ...
    for epoch in 0..=nb_epoch{
        let a= x1_train.broadcast_matmul(&w1.unsqueeze(1)?)?;
        println!("{}",a);
    }
    Ok(())
}
